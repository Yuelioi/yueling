...
class ImageStorage:
  ...
  def _validate_storage(self):
    ...
    for s in validation_result.errors:
      logging.getLogger(__name__).info(f"校验失败: {s}")
    # 输出统计信息
    ...
    if remove_file:
      ...
      if img_path.exists():
        try:
          ...
          logging.getLogger(__name__).info(f"文件删除失败: {img_path} - {str(e)}")
...
class ImageDatabase:
  ...
  def add_image(self, category: str, image_data: bytes, suffix: str, filename: str = "", tags: list[str] | None = None, uploader: str = "anonymous") -> ImageData:
    """添加图片到指定分类"""
...
def batch_add_images(db: ImageDatabase, image_folder: Path, tag_folder: Path):
  ...
  for file in image_folder.glob("*.*"):
    ...
    try:
      ...
      logging.getLogger(__name__).info(f"跳过 {file.name}: 标签文件不存在")
