...
def get_available_tools(user_role: str) -> list[ToolMeta]:
  available = []
...
async def dispatch(text: str, ctx: ToolContext) -> str:
  ...
  available = get_available_tools(ctx.role)
  if not available:
    return "当前没有可用的工具"
  ...
  # 写入语义记忆（如果文本匹配触发词）
  if should_write_semantic(text):
    try:
      await memory_manager.write_semantic(ctx.user_id, text)
    except Exception:
      pass
